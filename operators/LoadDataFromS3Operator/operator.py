...
class LoadDataFromS3Operator(BaseOperator):
    ...
    __name__ = "LoadDataFromS3Operator" 
    __version__ = "0.1.0"
    ...
    def operator_function(self, operator_model: OperatorModel):
        ...
        dataset_path_efs = f"{self.volume_mount_path}/datasets/{dataset_id}/"

        # Check if artifact already exists, before copying it from S3
        if os.path.exists(dataset_path_efs):
            self._logger.info(f"The {dataset_id} dataset already exists!")
        else:    
            # Copy dataset from S3 to Mount path
            err = s3_client.download_s3_folder(
                s3_folder=f"datasets/{dataset_id}/", 
                local_dir=dataset_path_efs
            )
            if err is None:
                self._logger.info(f"Dataset {dataset_id} successfully downloaded!")
            else:
